...
router.post('/import', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (homeworkId) {
    ...
    if (!hw) return res.status(404).json({ error: 'Target homework not found.' });
    hw.questions.push(...parsed.questions);
    await hw.save();
  } else {
    if (!lesson) return res.status(400).json({ error: 'lesson is required when creating a new homework.' });
  ...
  res.status(201).json({ homework: hw, imported: parsed.questions.length, warnings: parsed.errors });
}));
