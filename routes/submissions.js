...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!homeworkId || !Array.isArray(answers)) {
      return res
        .status(400)
        .json({ error: "homework and answers[] are required." });
    }
    ...
    if (existing) {
      return res
        .status(409)
        .json({ error: "You already submitted this homework." });
    }

    const answerByQuestion = new Map(
      answers.map((a) => [String(a.questionId), a.answerText]),
    );
    const noteByQuestion = new Map(
      answers.map((a) => [String(a.questionId), a.studentNote || ""]),
    );
